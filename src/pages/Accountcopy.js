...
const Account = () => {
  ...
  return (
    <>
      ...
      {singleProfile && (
        <>
        <section className='flex-top-row gap-20 px-40'>
        <section className=' flex-middle-down gap-8'>
          <div className="shadow-div">
            <div className='flex-middle-down'>
            <img
                  src={singleProfile.avatar}
                  alt={singleProfile.name}
                  className="h-32 w-32 object-cover object-center rounded-full"
                />
                <p className='title'>{singleProfile.name}</p>
                <p className=''>Guest</p>
            </div>
          </div>
          <div className="shadow-div">
        ...
        <section className='w-full flex flex-col gap-12'>
         <div>
          <p className='title'>{singleProfile.name}</p>
          <button className='round-outline-btn'>Edit</button>
         </div>
         <div>
         <ul className="flex flex-row gap-8">
              {menuItems.map((item) => (
                    <li id={item.id} onClick={() => handleClick(item.id)}>{item.title}</li>
              ))}
            </ul>
         </div>
         <div className='p-4 w-full'>
          <MyBookings singleProfile={singleProfile} timeDiff={timeDiff} dayjs={dayjs} setEditBooking={setEditBooking}/>
        ...
        <ul className=" flex flex-col gap-8 items-center text-sm pt-12 font-montS w-52 md:mt-20">
              {menuItems.map((item) => (
                <div>
                  <li
                    className={`cursor-pointer p-4 w-52 mx-auto flex flex-row items-center justify-center gap-2 text-center ${
                      active == item.id && "m-active"
                    } ${item.id === 1 && "bg-passionOrange drop-shadow-md"}`}
                    id={item.id}
                    key={item.id}
                    onClick={(id) => {
                      handleClick(id);
          ...
          <section className="  top-52 left-1/2 -translate-x-1/2 w-full  lg:w-1/2 flex flex-col-reverse md:flex-row items-center justify-center md:gap-24">
              ...
              <div className="text-right ">
                <Overlay>
                  ...
                  <form>
                    <input
                      type="text"
                      className="w-full h-10 border border-purpleBlack rounded-md px-4"
                    ...
                    <button
                      ...
                      onClick={(e) => {
                        e.preventDefault();
                        dispatch(
                          editProfile(singleProfile.name, newProfileImg)
                        );
                      }}
                      className="orangeBtn my-2 "
                    >
          ...
          <Menu title="New Venue" isActive={activeIndex == 1} amount={null}>
                        ...
                        <div className="p-4 md:p-6 flex flex-col justify-between md:w-1/2">
                          ...
                          <div className="flex flex-col gap-4">
                            <div className="flexR border-b border-gray-300 py-4 ">
                              ...
                              <p>
                                {timeDiff(booking.dateFrom, booking.dateTo)}{" "}
                                night
                              </p>
                            </div>
              ...
              <div className="flex flex-col gap-10">
                {singleProfile.venues.map((venue) => (
                  <>
                    ...
                    {fixVenue == venue && (
                      <EditVenue venue={venue} setfixVenue={setfixVenue} />
                    )}
                  </>
                ))}
              </div>
